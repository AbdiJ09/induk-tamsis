 <section id="quotes" class="py-20">
     <h1 class="text-center font-bold text-2xl my-10">Quotes</h1>
     <div class="swiper mySwiper">
         <div class="swiper-wrapper">
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Ki Joko Suyono</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             Jalani
                             Hidup dengan Penuh Keyakinan,
                             Tersenyum dan Bersabar dalam Menyelesaikan Masalah,
                             dan Hendaklah tidak Menyakiti orang Lain bila Kita Tidak Ingin Disakiti <i
                                 class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Ki Fuad Rohmansyah</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             Apapun yang kita lakukan, lakukan dengan sebaik mungkin <i
                                 class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Ki M. Aufaq</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             ALLAH BESERTA ORANG ORANG YANG BERBUAT BAIK <i class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Ki Prono Aji</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             Berbagi itu nagih <i class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/female.svg') }}"
                         class="block w-24 mx-auto rounded-full bg-white my-3 h-24" alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Nyi Kusdwiastuti</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             Bersabar dan bersyukur menuju ridho Alloh SWT <i class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="px-3">
                     <div class="">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Sri finayani</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             Rahasia kebahagiaan itu ada dalam 3 hal : bersabar, bersyukur, dan ikhlas <i
                                 class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Ki Mardisuroso</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             Menjadi tua itu keniscayaan, menjadi dewasa perlu belajar <i
                                 class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
             <div
                 class="swiper-slide overflow-hidden text-center bg-[#474747]/90 border-t-4 border-green-500  rounded-xl">
                 <div class="h-60 space-y-4">
                     <img src="{{ asset('image/male.svg') }}" class="block w-24 mx-auto rounded-full bg-white my-3 h-24"
                         alt="">
                     <div class="px-3">
                         <h5 class="text-gray-100 mt-3 font-bold capitalize text-lg">Miah</h5>
                         <p class="text-sm text-gray-200 text-center mt-3"><i class="fa-solid fa-quote-left"></i>
                             kata hati lebih alami daripada kata oranglain, oleh karenanya dengarkan kata hati daripada
                             kata orang lain <i class="fa-solid fa-quote-right"></i>
                         </p>
                     </div>
                 </div>
             </div>
         </div>
     </div>
 </section>
